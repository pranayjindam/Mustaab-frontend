// src/admin/pages/AdminReturnRequestsPage.jsx
...
const AdminReturnRequestsPage = () => {
  ...
  return (
    <div className="p-4 sm:p-6">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">Return / Exchange Requests</h1>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {requests.map((request) => {
          ...
          return (
            <div
              key={request._id}
              className="bg-white rounded-md border border-gray-200 p-4 flex flex-col justify-between"
            >
              {/* Request Info */}
              <div>
                ...
                <div className="space-y-1 text-sm text-gray-600">
                  ...
                  <p>
                    <strong>Product ID:</strong> {request.productId?._id || "N/A"}
                  </p>
                  <p>
                  ...
                  <p>
                    <strong>Pickup Address:</strong> {request.pickupAddress || "Not provided"}
                  </p>
                </div>

                <p className="text-sm text-gray-700 mt-3">
                  <strong>Status:</strong>{" "}
                  <span className={`px-2 py-1 rounded text-xs font-medium ${getStatusColor(statusText)}`}>
                    {statusText}
                  </span>
                </p>
              </div>
              ...
              <div className="mt-4 flex gap-3 justify-end">
                {statusText.toLowerCase() === "pending" ? (
                  <>
                    <button
                      className="px-3 py-2 rounded-md bg-green-600 text-white text-sm font-medium disabled:opacity-60"
                  ...
                  <span className="text-gray-500 text-sm italic">{statusText}</span>
                )}
              </div>
            </div>
          );
