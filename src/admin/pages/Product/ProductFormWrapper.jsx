...
export default function ProductForm({ product, onClose }) {
  ...
  const [form, setForm] = useState({
    ...
    tags: product?.tags || [],
    sizes: product?.sizes || [],
    colors: product?.colors?.map(c => ({ ...c })) || [{ name: "", image: "" }],
    price: product?.price || "",
    stock: product?.stock || "",
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("category[main]", form.category.main);
    if (form.category.sub) formData.append("category[sub]", form.category.sub);
    if (form.category.type) formData.append("category[type]", form.category.type);

    form.tags.filter(t => t.trim()).forEach((t) => formData.append("tags[]", t));
    form.sizes.filter(s => s.trim()).forEach((s) => formData.append("sizes[]", s));

    form.colors.forEach((c, i) => {
      formData.append(`colors[${i}][name]`, c.name);
      if (c.imageFile) formData.append(`colors[${i}][image]`, c.imageFile);
    });

    if (thumbnailFile) formData.append("thumbnail", thumbnailFile);
    imageFiles.forEach((file) => formData.append("images", file));

    ["isFeatured", "isReturnable", "isExchangeable"].forEach((key) =>
