    ...
    <div className="p-4 bg-white rounded-2xl shadow-sm border flex gap-4 items-center">
      ...
      <div className="flex-1">
        ...
        <div className="mt-2 text-sm text-gray-600 flex items-center gap-2">
          <span className="px-2 py-1 rounded-full text-xs bg-yellow-100">{order.status}</span>
          <button onClick={() => onOpen(order)} className="ml-auto text-sm underline">
            Details
          </button>
        </div>
      </div>
      ...
      <div className="bg-white w-full max-w-3xl rounded-2xl p-6 z-10 shadow-lg">
          ...
          <div className="mt-2 space-y-2">
            {order.orderItems.map((it) => (
              <div key={it._id} className="flex items-center gap-3">
                <img src={it.image || "/placeholder.png"} className="w-14 h-14 object-cover rounded-md" alt={it.name} />
                <div className="flex-1">
        ...
        <div className="mt-6 flex gap-3 items-center">
          ...
          <button
            ...
            className="ml-auto px-3 py-2 rounded bg-red-500 text-white"
          >
            Cancel Order
          </button>
          <button
...
export default function OrdersPage() {
 ...
 const orders = useMemo(() => {
  ...
  return myOrders ?? []; // ensure myOrders is accessed correctly
  ...
  useEffect(() => {
    ...
    s.on("orderUpdated", (order) => {
      toast(`Order ${order._id} status: ${order.status}`);
      if (isAdmin) refetchAll(); else refetchMy();
    });
  ...
  const handleUpdateStatus = async (id, status) => {
    try { await updateStatus({ id, status, token }).unwrap(); toast.success("Status updated"); }
    catch { toast.error("Failed to update status"); }
  };
  const handleCancel = async (id) => {
    try { await cancelOrder({ id, token }).unwrap(); toast.success("Order cancelled"); if (isAdmin) refetchAll(); else refetchMy(); }
    catch { toast.error("Cancel failed"); }
  };
  const handleReturn = async (id) => {
    try { await returnOrder({ id, token }).unwrap(); toast.success("Return processed"); if (isAdmin) refetchAll(); else refetchMy(); }
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid gap-3">
        ...
        {filtered.map((order) => <OrderCard key={order._id} order={order} onOpen={handleOpen} />)}
      </div>
